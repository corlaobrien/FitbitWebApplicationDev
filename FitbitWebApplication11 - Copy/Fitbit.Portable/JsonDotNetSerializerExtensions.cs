...
namespace Fitbit.Api.Portable
{
    internal static class JsonDotNetSerializerExtensions
    {
        ...
        /// <summary>
        /// GetHeartRateActivityDataList has to do some custom manipulation with the returned representation
        /// </summary>
        /// <param name="serializer"></param>
        /// <param name="timeSeriesDataJson"></param>
        /// <returns></returns>
        internal static TimeSeriesDataList GetHeartRateActivityDataList(this JsonDotNetSerializer serializer, string timeSeriesDataJson)
        {
            ...
            DateTime date = DateTime.Parse(((((JToken.Parse(timeSeriesDataJson)["activities-heart"]).First).First).First).ToString());
            var dataPoints = JToken.Parse(timeSeriesDataJson)[serializer.RootProperty];

            var result = new TimeSeriesDataList
            {
                DataList = (from item in dataPoints["dataset"]
                            select new TimeSeriesDataList.Data
                            {
                                DateTime = date.AddHours(DateTime.Parse(item["time"].ToString()).Hour).AddMinutes(DateTime.Parse(item["time"].ToString()).Minute).AddSeconds(DateTime.Parse(item["time"].ToString()).Second),
